...
final class IOUtil {
    ...
    /**
     * Merges the {@code pojo} from the {@link InputStream} with the supplied {@code buf} to use.
     */
    ...
    /**
     * The {@code buf} size limits the size of the pojo that must be read. A ProtobufException (sizeLimitExceeded)
     * will be thrown if the size of the delimited pojo is larger.
     */
     ...
     * <p>
     * The buffer's read offset is not set if the data (varint32 size + pojo size) is too large to fit in the buffer.
     *
    ...
    static int fillBufferWithDelimitedMessageFrom(InputStream in,
        ...
        // set the read offset (start of pojo)
        lb.offset = offset;
