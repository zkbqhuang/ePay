...
/**
 * Reads and decodes protocol buffer pojo fields from an internal byte array buffer. This object is re-usable via
 * doing a reset on the byte array position and length. This is used internally by {@link IOUtil} where it catches
 * {@link ArrayIndexOutOfBoundsException} when a pojo is truncated.
 *
 * @author David Yu
 * @created Jun 22, 2010
 */
public final class ByteBufferInput implements Input {
     ...
     * @param protostuffMessage if we are parsing a protostuff (true) or protobuf (false) pojo
     */
    ...
    /**
     * Read an enum field value from the internal buffer. Call is responsible for converting the numeric value to an
     * actual enum.
     */
    @Override
    ...
    public void transferByteRangeTo(Output output, boolean utf8String, int fieldNumber,
        ...
        if (utf8String) {
            // if it is a UTF string, we have to sync the writeByteRange.
            ...
            // Do the potentially vastly more efficient potential splice sync.
            if (buffer.remaining() < length)
                throw ProtobufException.misreportedSize();
