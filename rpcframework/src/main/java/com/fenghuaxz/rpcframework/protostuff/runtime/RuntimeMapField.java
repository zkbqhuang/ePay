...
abstract class RuntimeMapField<T, K, V> extends Field<T> {
    ...
    public RuntimeMapField(WireFormat.FieldType type, int number, String name, Tag tag,
    ...
    protected abstract K kFrom(Input input, MapSchema.MapWrapper<K, V> wrapper)
            throws IOException;

    protected abstract void vPutFrom(Input input, MapSchema.MapWrapper<K, V> wrapper,
                                     K key) throws IOException;
